...
/// |---|---|---|---|---|
/// | `_StopTab` の Column（節見出し＋カード＋リスト） | 縦 | **1.1** | しない | #240 |
/// | NEXT BUS カードの到着行（300px） | 横 | 1.15 | する（13文字。既定名なら 2.0） | #241 |
/// | 来週シートの到着行（303px） | 横 | 1.15 | する | #241 |
/// | 来週シートの行ヘッダ（311px） | 横 | 1.2 | しない | #242 |
/// | ホームの時刻表リストの到着行（335px） | 横 | 1.25 | する（既定名なら 2.0） | #241 |
/// | タブ（`Tab` の中の `Row`・4タブ） | 横 | **1.4** | しない | #243 |
/// | ホームの時刻表リストの行ヘッダ（343px） | 横 | 1.5 | しない | #242 |
///
