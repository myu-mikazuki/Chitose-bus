...
>
> **ただし「等倍で壊れる場所は無い」は現行の実データ（研究棟の到着地が
> 最大2件）が前提で、無条件に成り立つわけではない。** 到着地を3件に
> 増やすと実フォントでも 800×600 で溢れる（下の「2方向ある停留所でも
> 測ること」参照）。**測るときはデータの形（到着地の件数）も実データに
> 合わせること。**
...
> 見逃す。
>
> **さらに、`fixture` の到着地の件数を変えて測ること。** #251 の PR レビューで
> 指摘され、到着地の件数を2件→3件に変えて測り直したところ、**実フォントでも
> 800×600（幅が広くて縦が短い画面）で 16px 縦に溢れることが分かった**
> （375×667 は3件でも無傷）。**倍率・画面サイズだけを振ってデータの形
> （到着地の件数）を固定していると、この境目を見逃す。** いまの `kenkyuto`
> の fixture（`text_scale_probe_test.dart`）は現行の実データ（研究棟の
> 到着地は最大2件）に合わせてあるが、**`gas/Code.gs` のデータはリリースを
