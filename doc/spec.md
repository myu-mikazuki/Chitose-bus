...
>
> 正式名（`label`）が出るのは次の2箇所。
>
> | 出る場所 | 引き方 |
> |---|---|
> | 設定画面（`stop_settings_screen.dart`） | `BusStop` を直に持っているので `stop.label`。「タブ表示: ◯◯」と短縮名を併記する |
> | NEXT BUS / SCHEDULE の見出しの「◯◯ 発」（`home_screen.dart`） | `officialLabelOf`（#208） |
>
> **ID からしか引けない場所のための入口が `officialLabelOf`**（= `label`、
> 短縮名があってもそちらを使わない）。`labelOf` と対になる。
>
> 見出しの「◯◯ 発」は**いま見ている停留所を確定させるためのもの**なので、
> ここを短縮名にすると意味が無くなる。タブが省略されるぶんをここで補っている。
